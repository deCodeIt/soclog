...
if(setup('accessToken') && setup('name') && setup('email') && setup('prof_id') && setup('gender'))
{
	...
	$name = mysqli_real_escape_string($connect,htmlentities($_POST['name']));
	$email = mysqli_real_escape_string($connect,htmlentities($_POST['email']));
	$gender = mysqli_real_escape_string($connect,htmlentities($_POST['gender']));
	$prof_id = mysqli_real_escape_string($connect,htmlentities($_POST['prof_id']));
	$accessToken = mysqli_real_escape_string($connect,htmlentities($_POST['accessToken']));
